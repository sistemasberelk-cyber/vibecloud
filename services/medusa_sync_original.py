...
# ---------------------------------------------------------------------------
# Configuración (env vars — agregar a tu .env / Settings de VibeCloud)
# ---------------------------------------------------------------------------
...
class MedusaSyncService:
    ...
    def _map_product_payload(vibecloud_product: dict, tenant_id: str) -> dict:
        """
        Convierte el producto generado por GeminiService (schema interno VibeCloud)
        ...
        variants = vibecloud_product.get("variants") or [
            {
                ...
                "prices": [
                    {
                        "amount": int(round(vibecloud_product["price"] * 100)),  # Medusa usa centavos
        ...
        return {
            ...
            "metadata": {
                "tenant_id": tenant_id,
                "vibecloud_product_id": str(vibecloud_product["id"]),
    ...
    async def sync_product(
        ...
        if not medusa_id:
            existing = await self.find_by_vibecloud_id(tenant_id, vibecloud_product["id"])
            medusa_id = existing["id"] if existing else None

        if medusa_id:
            logger.info("Producto %s ya existe en Medusa (%s), actualizando.", vibecloud_product["id"], medusa_id)
            return await self.update_product(medusa_id, vibecloud_product, tenant_id)

        logger.info("Producto %s no existe en Medusa, creando.", vibecloud_product["id"])
        return await self.create_product(vibecloud_product, tenant_id)
